...
── RASPBERRY PI J8 HEADER ──────────────────────────────────────────────────
  ...
  Physical Pin 15 (GPIO 22)  → Button COLOR  (other leg to GND)
...
── BUTTONS ──────────────────────────────────────────────────────────────────
  ...
  PLAY   button: one leg → Pi Physical Pin 13 (GPIO 27)
                 other leg → Pi Physical Pin 14 (GND)
  COLOR  button: one leg → Pi Physical Pin 15 (GPIO 22)
                 other leg → Pi Physical Pin 14 (GND)
...
── LED STATES ───────────────────────────────────────────────────────────────
  ...
  Magenta (flash)  → error
  COLOR button     → cycles: Red→Green→Blue→Yellow→Cyan→Magenta→White→(Amber)
...
# Color cycle sequence for BTN_COLOR (excludes functional states)
COLOR_CYCLE = [
    (255,   0,   0),   # red
    (  0, 255,   0),   # green
    (  0,   0, 255),   # blue
...
COLOR_CYCLE_NAMES = ["Red", "Green", "Blue", "Yellow", "Cyan", "Magenta", "White"]
...
record_proc    = None
play_proc      = None
_color_index   = -1   # -1 = not in a custom color (idle/functional state owns LEDs)
...
#
# FIX: solid mode now uses a proper blocking wait with no spurious wakeups.
# The event is only set() when a *new* request arrives, so the worker only
# redraws when something actually changes — eliminating the flicker that
# occurred because the event was left set between button presses.
    ...
    while not _led_stop.is_set():
        ...
        try:
            if mode == "solid":
                pixels.fill(color)
                pixels.show()
                # KEY FIX: clear the event *after* we've drawn the current
                # state, then block until a genuinely new request arrives.
                # This means we only re-enter the loop when _post_led() fires,
                # so there are no redundant redraws between button presses.
                _led_event.clear()
                _led_event.wait(timeout=1.0)   # timeout lets _led_stop be checked
...
def action_color_cycle():
    """Advance through COLOR_CYCLE. Only active when not in a call/recording/playing."""
    global _color_index
    if in_call:
        print("[COLOR] In call — ignoring color cycle")
        return
    if is_recording:
        print("[COLOR] Recording — ignoring color cycle")
        return
    if play_proc and play_proc.poll() is None:
        print("[COLOR] Playing — ignoring color cycle")
        return

    _color_index = (_color_index + 1) % len(COLOR_CYCLE)
    color = COLOR_CYCLE[_color_index]
    name  = COLOR_CYCLE_NAMES[_color_index]
    print(f"[COLOR] Cycling to {name} {color}")
    _post_led("solid", color)
...
def action_record_stop():
    ...
    # Restore custom color if one was set, otherwise go idle
    if _color_index >= 0:
...
def main():
    ...
    print(f"  COLOR  (BCM22 / Pin 15) — cycle LED color")
    ...
    print(f"    Color cycle  = Red→Green→Blue→Yellow→Cyan→Magenta→White")
