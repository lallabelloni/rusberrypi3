...
── INSTALL DEPENDENCIES ─────────────────────────────────────────────────────
  ...
  baresip config must have the ctrl_tcp module loaded, e.g. a line:
      module                  ctrl_tcp.so
  in the [modules] section of ~/.baresip/config (or the -f config dir you pass).
── RUN ──────────────────────────────────────────────────────────────────────
  sudo python3 lamp_control.py

── ABOUT THE BARESIP ctrl_tcp PROTOCOL (read this if dial/hangup don't work) ─
  ctrl_tcp does NOT take plain-text "/dial ..." style commands. It speaks
  JSON, framed as a netstring: "<byte-length>:<json-payload>,"

  Command:   {"command": "dial", "params": "sip:alice@atlanta.com"}
  Sent as:   55:{"command": "dial", "params": "sip:alice@atlanta.com"},

  Responses/events come back the same way, e.g.:
  {"response": true, "ok": true, "data": "", "token": "..."}
  {"event": true, "class": "call", "type": "CALL_ESTABLISHED", ...}

  The previous version of this script sent raw "/dial sip:...\n" text,
  which baresip's ctrl_tcp parser silently rejects — so no dial command
  was ever actually executed (hence: no SIP packets, no LED feedback that
  reflected reality). This version sends/receives properly framed JSON.
"""
import time
import signal
import sys
...
#
# baresip ctrl_tcp accepts only ONE connection at a time, and it speaks
# JSON-over-netstring, NOT plain text. See module docstring above.
#
...
# ── Netstring framing helpers ───────────────────────────────────────────────
def _netstring_encode(payload: bytes) -> bytes:
    """djb netstring: '<byte-length>:<payload>,'"""
    return str(len(payload)).encode() + b":" + payload + b","
def _netstring_decode_buffer(buf: bytes):
    """
    Pull as many complete netstrings as possible out of buf.
    Returns (list_of_payload_bytes, remaining_buf).
    Tolerant of partial/garbled data: resyncs on framing errors instead
    of getting stuck.
    """
    messages = []
    while True:
        ...
        if not length_field.isdigit():
            # Garbage before a colon — drop it and look for the next one.
            buf = buf[colon + 1:]
            continue
        length = int(length_field)
        start = colon + 1
        end = start + length
        if len(buf) < end + 1:          # +1 for the trailing comma
...
def baresip_cmd(command, params=None, token=None):
    """
    Send a JSON command on the shared persistent socket, wrapped in
    netstring framing, e.g. for command="dial", params="sip:a@b.com":
        15:{"command": "dial", "params": "sip:a@b.com"},
    Returns True if the bytes were written to the socket (NOT a
    guarantee baresip executed it successfully — watch the console for
    the matching "[baresip] response: ok=..." line).
    """
    global _bs_socket
    msg = {"command": command}
    if params:
...
def start_event_listener():
    ...
    Reads netstring-framed JSON events/responses and dispatches them;
    commands are sent via baresip_cmd() on the same socket using _bs_lock.
    """
    def _listen():
                ...
                while True:
                    ...
                    for raw in msgs:
                        try:
                            ...
                            print(f"[baresip] bad JSON from baresip: {e}")
                            continue
                        ...
                        if obj.get("event"):
                            ...
                            print(f"[baresip] unrecognized message: {obj}")
            except Exception as e:
